...
newgrp libvirtd     # ????? TODO ?????

echo  $(lxc exec maas -- sudo -u maas sh -c "cat /var/lib/maas/.ssh/id_rsa.pub") >> ~ubuntu/.ssh/authorized_keys >> $LOG 2>&1
...
git clone http://github.com/marosg42/deploy-storage.git >> $LOG 2>&1
deploy-storage/scripts/define_juju.sh >> $LOG 2>&1
