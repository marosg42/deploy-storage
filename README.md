...
sudo apt update
sudo apt install libvirt-bin qemu-kvm virtinst -y
...
```ENG
sudo apt remove lxd lxd-client --purge -y
...
sudo brctl addbr maasbr1
sudo brctl addbr maasbr2
sudo brctl addbr maasbr3

bash -c "printf \"auto maasbr0\niface maasbr0 inet static\n    address 192.168.110.1/24 \n    bridge_stp off \n\""|sudo tee /etc/network/interfaces.d/mg.cfg
bash -c "printf \"auto maasbr1\niface maasbr1 inet static\n    address 192.168.111.1/24 \n    bridge_stp off \n\""|sudo tee -a /etc/network/interfaces.d/mg.cfg
...
sudo iptables -t nat -A POSTROUTING -s 192.168.110.0/24 ! -d 192.168.110.0/24 -m comment --comment "network maasbr0" -j MASQUERADE
...
## start MaaS container and configure networking

```ENG
lxc launch ubuntu: --profile maas maas

lxc exec maas -- bash -c "echo \"network: {config: disabled}\"| sudo tee /etc/cloud/cloud.cfg.d/99-disable-network-config.cfg"
...
\"|sudo tee /etc/netplan/mg.yaml"

lxc exec maas -- netplan apply
```

## proxy in MaaS container
